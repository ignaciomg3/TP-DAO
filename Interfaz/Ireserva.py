...
def ventana_registrar_reserva(root, db):
    ...
    ttk.Label(ventana, text="Habitación:").pack()
    habitacion_var = tk.StringVar(ventana)
    habitacion_var.set(next(iter(habitaciones_opciones)))  # Establecer opción predeterminada
    habitacion_menu = ttk.OptionMenu(ventana, habitacion_var, *habitaciones_opciones.keys())
    habitacion_menu.pack()

    # Campo para seleccionar cliente
    ttk.Label(ventana, text="Cliente:").pack()
    cliente_var = tk.StringVar(ventana)
    cliente_var.set(next(iter(clientes_opciones)))  # Establecer opción predeterminada
    cliente_menu = ttk.OptionMenu(ventana, cliente_var, *clientes_opciones.keys())
    cliente_menu.pack()

    # Campos de entrada para las fechas y la cantidad de personas
    ttk.Label(ventana, text="Fecha de Inicio (YYYY-MM-DD):").pack()
    fecha_inicio_entry = ttk.Entry(ventana)
    fecha_inicio_entry.pack()

    ttk.Label(ventana, text="Fecha de Fin (YYYY-MM-DD):").pack()
    fecha_fin_entry = ttk.Entry(ventana)
    fecha_fin_entry.pack()

    ttk.Label(ventana, text="Cantidad de Personas:").pack()
    cant_personas_entry = ttk.Entry(ventana)
